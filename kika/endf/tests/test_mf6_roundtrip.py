...
That is asserted on four committed fixtures which between them carry every LAW
that occurs on a tape on this machine, and — ``tape``-marked — on the full
evaluations they were cut from.
...
**LAW=5 is not tested against a tape, because there is no tape.** Charged-particle
elastic scattering does not occur in the neutron sublibrary, and this machine
holds no charged-particle sublibrary. It is gated on a synthetic section written
by kika's own emitter, which proves the record walk is self-consistent and
proves nothing about the format. That distinction is the point of the test's
name, and it must not be quietly upgraded.
"""
...
def test_the_fixtures_between_them_carry_every_law_on_a_tape_here():
    ...
    notices that a law lost its only witness.
    """
    seen = set()
    for path in FIXTURES.values():
        for section in all_sections(path).values():
    ...
    assert seen == {-15, -5, 0, 1, 2, 3, 4, 6, 7}, (
        f"law coverage drifted: {sorted(seen)}. LAW=5 is absent on purpose — "
        f"no tape on this machine carries one."
    )
...
def test_law5_roundtrips_through_our_own_emitter():
    """LAW=5 has no witness on this machine — this is self-consistency only.

    Charged-particle elastic scattering does not occur in the neutron
    sublibrary, and there is no charged-particle sublibrary here. So this test
    proves the LAW=5 reader and writer agree with each other, and **nothing
    about whether either agrees with ENDF-6**. If a charged-particle tape ever
    lands on this machine, replace this with a fixture cut from it.
    """
