...
WHY IT LIVES IN THE LIBRARY (moved 2026-08-24). It used to be
``scripts/mf34_cross_reader.py``, read only by the chi^2. The sampler needs the
same blocks -- an ensemble drawn from an ``_a0cross`` tape without them is drawn
from a distribution the file does not describe -- and two readers of the same
bytes is the shape of every failure this track has had (§L, §L3, §L9: "a joint
was certified that was not the joint being shipped"). So there is one reader and
both callers import it. ``scripts/mf34_cross_reader`` is now a re-export and
raises loudly, not silently, against a stale wheel.
"""
...
def read_mf34_split(
    ...
    *,
    ...
    endf : parsed tape, optional
        Read MF34 out of this instead of parsing *path* again. Defaults to
        ``None``, i.e. today's behaviour.

        ⚠ **This function MUTATES the section it reads**: it removes the a₀
        sub-subsections from the self subsection before calling
        ``to_ang_covmat``, because leaving them in projects each onto
        ``union(magnitude, shape)`` — a ~3020² dense matrix per order, retained
        for the load's lifetime, consumed by nothing. When you pass your own
        *endf* you get that mutation, and for the joint builder that is the
        point: what it hands on to ``decodeCovarianceSuite`` afterwards is the
        L≥1 remainder, which is exactly the object today's sampler assembles.
        Do not reuse the object afterwards expecting the a₀ blocks to be there.
