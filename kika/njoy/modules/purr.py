"""PURR — calculate probability tables in the unresolved range."""
...
def generate(p: dict) -> Lines:
    ...
    if nunx is not None or p.get("iprint") is not None:
        card2_parts.append(str(iprint if iprint is not None else "1"))
        if nunx is not None:
            card2_parts.append(str(nunx))
...
def parse(card_lines: list[str]) -> dict:
    """Parse PURR card lines into a parameter dict."""
    from ._base import parse_card_values

    # Card 1: nendf nin nout
    c1 = parse_card_values(card_lines[0])
    # Card 2: matd ntemp nsigz nbin nladr [iprint] [nunx]
